...
st.set_page_config(page_title="vTools Watermark Editor Soft Overlay", layout="wide")
st.title("📷 vTools Pro: Sửa Chữ & Làm Mờ Mảng Xám Nền")
...
st.subheader("2. Tùy chỉnh độ mờ mảng xám & Tọa độ")
col1, col2 = st.columns(2)
with col1:
    st.markdown("**Độ đục mảng xám nền (Gợi ý: 0.10 - 0.20 để mảng xám mờ nhẹ tự nhiên):**")
    shadow_opacity = st.slider("Độ đậm mảng xám (Opacity):", min_value=0.0, max_value=0.5, value=0.15, step=0.01)
    
    st.markdown("**Vùng xử lý:**")
...
if uploaded_files:
    ...
    if preview_img is not None:
        ...
        # Khung đỏ xem trước vùng khoanh
        cv2.rectangle(
            preview_img, 
...
def process_vtools_ultra_soft(img, cx, cy, cw, ch, l1_str, l2_str, f_size, l_spacing, tx, ty, opacity):
    """
    Xóa dòng chữ cũ và hòa trộn lớp xám mờ cực nhẹ (Ultra-soft Alpha Overlay),
    giúp mảng xám mờ đi rõ rệt và hiển thị cảnh vật đằng sau tự nhiên.
    """
    ...
    # 1. Xóa nét chữ cũ nhẹ nhàng bằng Inpainting để loại bỏ hoàn toàn chữ cũ
    ...
    # 2. Tạo mảng xám siêu nhẹ (Ultra-transparent Soft Mask)
    overlay = img.copy()
    cv2.rectangle(overlay, (x1, y1), (x2, y2), (0, 0, 0), -1)

    # Làm mượt viền xung quanh mảng xám bằng Gaussian Blur lớn
    ...
    # 3. Chèn 2 dòng chữ mới nét và rõ ràng
    ...
    # Viền bóng chữ thanh mảnh giúp chữ trắng dễ đọc trên nền xám siêu mỏng
    shadow_layer = Image.new("RGBA", pil_img.size, (0, 0, 0, 0))
    draw_shadow = ImageDraw.Draw(shadow_layer)

    if l1_str:
        draw_shadow.text((int(tx) + 1, y_line1 + 1), l1_str, fill=(0, 0, 0, 180), font=font)
    if l2_str:
        draw_shadow.text((int(tx) + 1, y_line2 + 1), l2_str, fill=(0, 0, 0, 180), font=font)

    shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(radius=0.5))

    # Lớp chữ trắng chuẩn vTools
...
if uploaded_files and st.button("🚀 Bắt Đầu Xử Lý Hàng Loạt"):
    ...
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        for idx, uploaded_file in enumerate(uploaded_files):
            ...
            final_img = process_vtools_ultra_soft(
                img, 
                clean_x, 
                clean_y, 
                clean_w, 
    ...
    st.success("✅ Đã xử lý xong! Mảng xám đã mờ đi nhiều, cảnh phía sau hiện rõ rất tự nhiên.")
    st.download_button(
        label="📥 Tải về file ZIP kết quả",
        data=zip_buffer.getvalue(),
        file_name="vtools_mang_xam_mo_nhat.zip",
